"""API routes for Content Hub entities (articles, podcasts, videos, news)."""

from typing import Optional

from fastapi import APIRouter, Depends, HTTPException, Query, Response

from app.features.auth.dependencies import get_current_user, get_admin_user
from app.features.auth.models import User
from app.deps.tenant import tenant_dependency
from app.features.core.database import get_db, AsyncSession
...
router = APIRouter(prefix="/content", tags=["community-content"])
...
async def list_articles_api(
    ...
    """List articles - now with explicit tenant_id parameter."""
    tag_list = [tag.strip() for tag in tags.split(",")] if tags else None
    items, total = await service.list_content(search=search, category=category, tags=tag_list, limit=limit, offset=offset)
    return {
...
async def create_article_api(
    ...
    item = await service.create_content(payload.model_dump(), current_user)
    if not item:
...
async def update_article_api(
    ...
    item = await service.update_content(content_id, payload.model_dump(exclude_unset=True), current_user)
    if not item:
...
async def list_podcasts_api(
    ...
    """List podcasts - now with explicit tenant_id parameter."""
...
async def list_videos_api(
    ...
    """List videos - now with explicit tenant_id parameter."""
...
async def list_news_api(
    ...
    """List news - now with explicit tenant_id parameter."""
...
async def record_engagement_api(
    ...
    current_user: User = Depends(get_current_user),
    service: ContentEngagementService = Depends(get_content_engagement_service),
):

...
from app.features.administration.smtp.models import (
    SMTPConfigurationCreate, SMTPConfigurationUpdate, SMTPConfigurationResponse, SMTPSearchFilter,
    SMTPStatus
)
...
router = APIRouter(prefix="/administration/smtp", tags=["smtp"])
...
async def update_smtp_field_api(config_id: str, field_update: dict = Body(...), db: AsyncSession = Depends(get_db), tenant: str = Depends(tenant_dependency), current_user: User = Depends(get_current_user)):
    ...
    updated_config = await service.update_smtp_field(config_id, field, value)
...
# --- SMTP Configuration Action Routes ---

@router.post("/validate/name")
async def validate_smtp_name(request: Request, db: AsyncSession = Depends(get_db), tenant: str = Depends(tenant_dependency), current_user: User = Depends(get_current_user)):
    """Validate SMTP configuration name."""
    try:
        form_handler = FormHandler(request)
        await form_handler.parse_form()

        name = form_handler.form_data.get("name", "").strip()
        if not name:
        ...
        return HTMLResponse('<span class="valid-feedback d-block">Configuration name looks good</span>')
    except Exception:
        return HTMLResponse("")
...
async def smtp_list(request: Request, db: AsyncSession = Depends(get_db), tenant: str = Depends(tenant_dependency), current_user: User = Depends(get_current_user)):
    return templates.TemplateResponse("administration/smtp/list.html", {"request": request})
...
async def smtp_create(request: Request, db: AsyncSession = Depends(get_db), tenant: str = Depends(tenant_dependency), current_user: User = Depends(get_current_user)):
    ...
    try:
        logger.info("Starting SMTP configuration creation process")
        ...
        service = SMTPConfigurationService(db, tenant)
        new_config = await service.create_smtp_configuration(config_data)

        logger.info(f"Successfully created SMTP configuration with ID: {new_config.id}")
...
async def smtp_update(request: Request, config_id: str, db: AsyncSession = Depends(get_db), tenant: str = Depends(tenant_dependency), current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        service = SMTPConfigurationService(db, tenant)
        updated_config = await service.update_smtp_configuration(config_id, config_data)
...
async def smtp_delete(config_id: str, db: AsyncSession = Depends(get_db), tenant: str = Depends(tenant_dependency), current_user: User = Depends(get_current_user)):
    """Delete SMTP configuration."""
    service = SMTPConfigurationService(db, tenant)
    ...
    await db.commit()
    return {"success": True, "message": f"SMTP configuration '{config_name}' deleted"}
